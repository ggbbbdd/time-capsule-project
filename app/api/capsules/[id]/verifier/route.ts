  ...
  try {
    ...
    try {
      ...
      if (capsuleCheck.rows[0].status !== 'draft') {
        throw new Error("초안(draft) 상태에서만 인증자를 초대할 수 있습니다.");
      }
      ...
      if (friendId === capsuleCheck.rows[0].owner_id) {
        throw new Error("자기 자신은 인증자가 될 수 없습니다.");
      }
      ...
      // 역할 추가 (Co-Signer)
      await client.query(
        `INSERT INTO "CAPSULE_ROLE" (capsule_id, user_id, role_type) VALUES ($1, $2, 'verifier')`,
        [capsuleId, friendId]
      );
      ...
      return NextResponse.json({ message: "친구를 인증자로 초대했습니다!" }, { status: 200 });
